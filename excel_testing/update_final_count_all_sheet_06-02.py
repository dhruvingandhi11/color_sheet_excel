...
# DB_NAMES = ["trailer_yard__1-02-06-06-49-22"]
...
for DB_NAME in DB_NAMES:
                        ...
                        if 'camera' in each_frame:
                            ...
                            if str(parking_id) == each_frame['location']:
                                ...
                                if (mongo_parking_status == 'DOCKED' or mongo_parking_status ==  'PARKED' or mongo_parking_status == 'MOVING') and actual_final_park_status == 'parked':
                                    ...
                                    # print(index,'+++')
                                    false_counter += 1
                                    cases_types[false_counter] = {'actual_loc':parking_id,'actual_time':actual_final_time, 'actual_status':actual_final_park_status, 'mongo_time':mongo_event_time, 'mongo_status':mongo_parking_status}
    ...
    types_of_cases_count['cases_types'] = cases_types
    final_s.update({DB_NAME:types_of_cases_count})
    
print(f"{final_s = }")

all_db_sheet['final'] = pd.DataFrame.from_dict(final_s)  
all_db_sheet['final'] = all_db_sheet['final'].T
